...
Using scp to transfer files from local machine let's say file.txt to HiPerGator,  first cd  to the location where file.txt exists. 
...
Then type the following in terminal: 
 ...
 Replace GROUPNAME and USERNAME with your account information. For more information, [see this post]()
 For transfering ALL files with extension .txt, use the wildcard *  as follows:
